...
from src.messages.pack import unpack, pack
from src.messages.body import MsgType, ErrorCode, PEERS_body, ERROR_body, REPRT_body
from src.util.util import File, Peer, encode_peers

hostname = socket.gethostname()
HOST_DEFAULT = socket.gethostbyname(hostname)
files = list()
file_timeout = 5 * 60  # seconds
...
async def process_REPRT(data, client_socket):
    print(f"Got REPRT from {client_socket.getsockname()}")
